...
session.renderer.header_level_by_type =  {'Module': 2,'Class': 3,'Method': 4,'Function': 3,'Variable': 4}
...
m = modules[0]
session.renderer.render_single_page(open(f"docs/Reference/Python/index.md", 'w'), [m], page_title=m.name)

for i, m in enumerate(modules[1:]):
    name = ".".join(m.name.split(".")[1:]).replace("_","").capitalize()
    print(name)
    session.renderer.render_single_page(open(f"docs/Reference/Python/{i}_{name}.md", 'w'), [m], page_title=name)
