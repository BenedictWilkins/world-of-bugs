...
namespace WorldOfBugs {
    ...
    public class ReflectionActuatorComponent : Unity.MLAgents.Actuators.ActuatorComponent {
        ...
        public override ActionSpec ActionSpec {
            get {
                return ActionSpec.MakeDiscrete(0);    // TODO update action spec properly
            }
        }
    ...
    public class ReflectionActuator : IActuator {
        ...
        public ReflectionActuator(IHeuristicProvider heuristic,
                                  Action[] discrete_action_methods, Action<float>[] continuous_action_methods) {
        ...
        public void OnActionReceived(ActionBuffers
                                     buffer) { // executes discrete actions first, then continuous actions. continuous actions are executed in order of specification
            ...
            if(_heuristic != null) {
                Heuristic(
                    buffer); // use the heuristic, it is up to the agent to either replace the action or leave it.
            }
