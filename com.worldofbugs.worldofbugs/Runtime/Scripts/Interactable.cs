...
public abstract class Interactable : MonoBehaviour {
    ...
    protected static Dictionary<Collider, Interactor> Interactors = new
    Dictionary<Collider, Interactor>();
    ...
    public static void Interact(Collider collider) {
        if(Interactors.ContainsKey(collider)) {
            ...
            throw new KeyNotFoundException(
                $"Invalid interaction, collider {collider} was not recognised, did you forget to register it?");
        }
    }
    ...
    public void Awake() {
        ...
        if(triggers.Any(x => x.isTrigger)) {
            Debug.LogWarning(
                $"Found Collider(s) but no triggers. This interactable {gameObject} cannot be interacted with");
        }
    }
