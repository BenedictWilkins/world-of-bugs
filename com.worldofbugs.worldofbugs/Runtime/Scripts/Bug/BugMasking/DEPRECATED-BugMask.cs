...
namespace WorldOfBugs {
    ...
    public class BugMask : MonoBehaviour {
        ...
        public static BugMask Instance {
            get {
                return _instance;
            }
        }
        ...
        protected Dictionary<int, MaskData> _masks = new
        Dictionary<int, MaskData>(); // map from RenderTexture GUID to pixel array (mask)

        public int[] Mask(Camera camera) {
            BugMaskReplacementShader replacementShader =
                camera.gameObject.GetComponent<BugMaskReplacementShader>();
            ...
            if(_frame != Time.frameCount) {
                ...
                _masks[_id] = new MaskData() {
                    mask = _mask, frame = _frame
                };
                return _mask;
            } else {
        ...
        public int[] GenerateMask(RenderTexture renderTexture) {
            ...
            for(int i = 0; i < pixs.Length; i++) {
                pixs[i] = 0xFFFF * rgbs[i].r + 0xFF * rgbs[i].g +
                          rgbs[i].b; // TODO this must match the BugType conversion operator implementation...
            }
