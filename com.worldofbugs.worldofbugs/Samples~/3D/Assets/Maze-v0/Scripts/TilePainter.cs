...
public class TilePainter : MonoBehaviour {
    ...
    void OnValidate() {
        ...
        bounds.center = new Vector3((width * gridsize) * 0.5f - gridsize * 0.5f, 0f,
                                    (depth * gridsize) * 0.5f - gridsize * 0.5f);
        bounds.size = new Vector3(width * gridsize, 0f, (depth * gridsize));
    }

    public Vector3 GridV3(Vector3 pos) {
        Vector3 p = transform.InverseTransformPoint(pos) + new Vector3(gridsize * 0.5f, 0f,
                    gridsize * 0.5f);
    ...
    public void Drag(Vector3 mouse, TileLayerEditor.TileOperation op) {
        ...
        if(this.ValidCoords((int)cursor.x, (int)cursor.z)) {
            if(op == TileLayerEditor.TileOperation.Sampling) {
                UnityEngine.Object s = PrefabUtility.GetPrefabParent(tileobs[(int)cursor.x,
                                       (int)cursor.z]);
            ...
            if(cursor.z == -1 && cursor.x >= 0 && cursor.x < palette.Count) {
                if(op == TileLayerEditor.TileOperation.Sampling ||
                        op == TileLayerEditor.TileOperation.Drawing) {
    ...
    public void OnDrawGizmos() {
        ...
        if(focused) {
            ...
            Gizmos.DrawRay((cursor * gridsize) + Vector3.forward * -49999f,
                           Vector3.forward * 99999f);
        ...
        Gizmos.DrawWireCube(new Vector3((width * gridsize) * 0.5f - gridsize * 0.5f, 0f,
                                        (depth * gridsize) * 0.5f - gridsize * 0.5f),
